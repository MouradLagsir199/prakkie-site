(function () {
  ...
  function renderCatalog(report,cat){
    ...
    (pipelines||[]).forEach(function(p){var weekly=p.name==='drugstores';var stale=p.completedAt && Date.now()-Date.parse(p.completedAt)>(weekly?8*24:36)*3600000;var success=p.status==='success';var n=el('div',null,'card pipeline');n.append(el('span',stale?'Verversing te oud':success?'Geslaagd':p.status==='failure'?'Mislukt':p.status==='cancelled'?'Geannuleerd':'Onbekend','badge'+(stale||!success?' warn':'')),el('strong',p.name==='drugstores'?'Kruidvat & Etos':'Supermarkten'),el('p',(weekly?'Wekelijks · maandagnacht. ':'Dagelijks. ')+'Laatste afgeronde poging: '+when(p.completedAt)));if(p.url&&/^https:\/\/github\.com\/MouradLagsir199\/prakkie\/actions\/runs\/\d+$/.test(p.url)){var a=el('a','Bekijk verversing ↗');a.href=p.url;a.target='_blank';a.rel='noopener noreferrer';n.append(a);}$('pipeline-cards').append(n);});
